import numpy as np #线性代数工具库
from sklearn import datasets
import matplotlib.pyplot as plt


class Config:
    nn_input_dim = 2 #数组输入的维度是2
    nn_output_dim = 2#数组输出的维度是2
    epsilon = 0.01  # 梯度下降学习步长
    reg_lambda = 0.01  # 修正的指数


def generate_data():
    np.random.seed(0)#伪随机数的种子0
...
def predict(model, x):
    ...
    exp_scores = np.exp(z2)#这两步表示输出层的激活函数为softmax函数
...
def build_model(X, y, nn_hdim, num_passes=20000, print_loss=False):
    ...
    for i in range(0, num_passes):
        ...
        #个人认为下面两行代码完全没有必要存在
        dW2 += Config.reg_lambda * W2#w2梯度增量的修正
        dW1 += Config.reg_lambda * W1#w1梯度增量的修正
        #更新权值和偏置
